...
- Drawing defaults to automatic pointer behavior: stylus/Apple Pencil and mouse input draw without scrolling, while finger touch scrolls vertically. Touch Draw lock remains available when finger drawing is intentional.
- Equation solving/completion requests structured intermediate LaTeX for every meaningful step; each step is rendered as a formula block before the completed equation or final answer, with legacy string-only AI responses still accepted safely.
- Vision-capable equation models receive a bounded set of validated inline PNG/JPEG images from the owned note in addition to the temporary handwriting snapshot. Remote image URLs, invalid data URLs, and oversized image payloads are never fetched or forwarded; text-only models receive text alone.
- Fullscreen PDF, EPUB, MOBI, PPTX, TXT, SRT, and VTT readers can reuse the note annotation canvas after Save as Note Source completes. The fullscreen book and its note's supported Original preview share the same note, source-file, book, and page annotation scope, so source-page ink is not duplicated between views. Drawings, handwriting, highlighter strokes, text boxes, and comments are page-scoped database annotations; Ask AI and selection-based source highlights remain available, and the OneDrive original is never modified.

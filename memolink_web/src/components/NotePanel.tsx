...
export function NotePanel({
  ...
  return (
    <>
      <div
        className="w-[4px] cursor-col-resize bg-[var(--ml-bg-panel)] hover:bg-indigo-600/40 rounded-full self-stretch transition"
        onMouseDown={onResizeStart}
      />
      <div
        className="flex flex-col bg-[var(--ml-bg-base)] border border-[var(--ml-bg-panel)] rounded-2xl overflow-hidden"
        style={{ width }}
      >
        {/* Tabs row */}
        <div className="flex overflow-x-auto border-b border-[var(--ml-bg-panel)] shrink-0 bg-[var(--ml-bg-bar)]">
          {tabs.map((tab, i) => (
            <div
              key={i}
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-xs cursor-pointer shrink-0 border-b-2 transition-all ${
                i === activeIndex
                  ? "border-indigo-500 text-white bg-[var(--ml-bg-base)]"
                  : "border-transparent text-gray-500 hover:text-gray-300 hover:bg-[var(--ml-bg-base)]"
              }`}
            >
              ...
              <button
                ...
                className="text-gray-600 hover:text-gray-300 leading-none transition w-3.5 h-3.5 flex items-center justify-center rounded-sm hover:bg-[var(--ml-bg-hover)]"
              >
                ×
              </button>
            </div>
        ...
        <div className="flex flex-col flex-1 p-4 overflow-hidden">
          ...
          <input
            ...
            className="w-full bg-transparent border-b border-[var(--ml-bg-panel)] pb-1 mb-3 text-sm text-indigo-200 focus:outline-none focus:border-indigo-500"
          />

          {/* Raw / Formatted tabs */}
          <div className="flex gap-1 border-b border-[var(--ml-bg-panel)] mb-3">
            {(["raw", "formatted"] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setNoteTab(tab)}
                className={`px-3 py-1 text-xs rounded-t-md capitalize transition ${noteTab === tab ? "bg-[var(--ml-bg-panel)] text-white" : "text-gray-500 hover:text-white"}`}
              >
                {tab}
              </button>
            ))}
            ...
            <>
              ...
              <textarea
                ...
                className={`flex-1 w-full bg-transparent text-sm text-gray-100 resize-none focus:outline-none rounded-md p-2 border transition ${
                  isRecording ? "border-red-500/40" : "border-[var(--ml-bg-panel)] focus:border-indigo-600/50"
                }`}
          ...
          {/* Formatted preview */}
          {noteTab === "formatted" && (
            <div className="flex-1 overflow-auto border border-[var(--ml-bg-panel)] rounded-md p-3 bg-[#080809]">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1 rounded-full text-xs border border-[var(--ml-bg-hover)] text-gray-400 disabled:opacity-30 shrink-0"
            >
              Discard
            </button>
            <button

...
interface EmailSingleFolderListProps {
...
export function EmailSingleFolderList({
  account,
  folder,
  onOpenEmail,
  ...
  const cacheKey = `${account.id}:${folder}`;
  ...
  async function load(pageToken?: string | null, silent = false) {
    ...
    if (isFreshLoad) {
      if (!silent) {
        const cached = readEmailCache<{ emails: BrowseEmailResult[]; nextPageToken: string | null }>(cacheKey);
        if (cached) {
    ...
    setError(null);
    try {
      const res = await browseEmails({ folder, emailAccountId: account.id, pageToken, pageSize: account.page_size });
      setEmails((prev) => {
        ...
        if (isFreshLoad) writeEmailCache(cacheKey, { emails: next, nextPageToken: res.next_page_token });
        return next;
      });

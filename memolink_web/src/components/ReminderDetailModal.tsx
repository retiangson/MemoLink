...
export function ReminderDetailModal({ item, onClose, onSave, onDelete, onToggleDone }: ReminderDetailModalProps) {
  ...
  return (
    ...
    >
      <div
        className={`bg-[#1a1a24] border border-[var(--ml-bg-hover)] rounded-2xl w-full shadow-2xl flex flex-col max-h-[90vh] ${linkedEmail ? "max-w-lg" : "max-w-md"}`}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--ml-bg-hover)]">
          <div className="flex items-center gap-2">
            {item.type === "ai" ? (
              ...
              <span className="flex items-center gap-1 text-[10px] font-semibold text-gray-400 bg-[#252533] border border-[var(--ml-bg-hover)] px-2 py-0.5 rounded-full uppercase tracking-wider">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 16 16">
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-[var(--ml-bg-surface)] border border-[var(--ml-bg-hover)] rounded-xl px-3 py-2.5 text-sm text-gray-200 placeholder-gray-700 outline-none focus:border-indigo-500 transition resize-none leading-relaxed"
            />
          </div>
            ...
            <div className={`flex items-center gap-2 text-xs rounded-xl px-3 py-2 ${
              isOverdue
                ...
                : isToday
                  ...
                  : "bg-[var(--ml-bg-surface)] border border-[var(--ml-bg-hover)] text-gray-500"
            }`}>
          ...
          {/* Recurrence / Google sync info */}
          {(item.recurrence_rule || item.source === "google") && (
            <div className="flex items-center gap-2 flex-wrap text-xs">
              {item.recurrence_rule && (
                <span className="flex items-center gap-1.5 bg-[var(--ml-bg-surface)] border border-[var(--ml-bg-hover)] text-gray-400 px-2.5 py-1 rounded-lg">
                  ...
                  {describeRecurrence(item.recurrence_rule)}
                </span>
              )}
              {item.source === "google" && (
                <span className="text-[10px] text-blue-300 bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded-lg">
          ...
          {linkedEmail && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 px-3 py-2 bg-[var(--ml-bg-surface)] border border-[var(--ml-bg-hover)] rounded-xl">
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition border ${
              item.done
                ? "bg-[var(--ml-bg-surface)] border-[var(--ml-bg-hover)] text-gray-400 hover:text-gray-200 hover:border-[#3a3a4a]"

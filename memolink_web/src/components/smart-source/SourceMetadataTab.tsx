...
export function SourceMetadataTab({ source, localCacheStatus, rawContent, noteId, uploadDisabled, onUploadComplete }: Props) {
  return (
    <div className="h-full overflow-y-auto p-5">
      {noteId && (
        <div className="mb-4">
          <SourceUploadButton noteId={noteId} disabled={uploadDisabled} onComplete={onUploadComplete ?? (() => {})} />
        </div>
      )}
      {!source ? (
        <>
          <div className="text-sm text-gray-500"><p>No original source is linked to this note.</p></div>
          <details className="mt-4"><summary className="cursor-pointer text-xs text-gray-500">Raw editor content</summary><pre className="mt-2 whitespace-pre-wrap text-xs">{rawContent}</pre></details>
        </>
      ) : (
        <>
          ...
          <dl className="grid grid-cols-[minmax(130px,0.4fr)_1fr] gap-x-4 gap-y-3 text-xs">
            {[
              ...
              <React.Fragment key={label}><dt className="text-gray-500">{label}</dt><dd className="break-all text-gray-300">{value}</dd></React.Fragment>
            ))}
          </dl>

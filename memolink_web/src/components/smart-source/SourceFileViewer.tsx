...
export function SourceFileViewer({ noteId, source, bookId, annotations, onAnnotationsChanged, onCacheStatus }: Props) {
  ...
  return (
    <div className="relative h-full overflow-hidden bg-[#171720]">
      ...
      {cache.objectUrl && isPdf && <PdfSourceViewer noteId={noteId} sourceFileId={source.id} bookId={bookId} objectUrl={cache.objectUrl} annotations={annotations} onAnnotationsChanged={onAnnotationsChanged} />}
      {cache.objectUrl && isImage && (
        <div className="flex h-full items-center justify-center overflow-auto p-4">
          <div className="relative inline-block max-h-full max-w-full">
            ...
            <AnnotationCanvas noteId={noteId} sourceFileId={source.id} bookId={bookId} annotations={annotations} onPersisted={onAnnotationsChanged} />
          </div>
        </div>
      )}
